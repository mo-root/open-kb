...
/** Sniff one file into the drift shape: top-level entities (sweep, swarm) or
 *  the kernel wrapper's `result`. Anything else is refused by name. */
function readRun(path: string): DriftMap {
...
// The sweep's own display rule: noise rows are judged off the map, so they do
// not drift on or off it either. Excluded here, not in core — the pure diff
// compares whatever rows it is handed.
const denoise = (m: DriftMap): DriftMap => ({ entities: m.entities.filter((e) => e.kind !== "noise"), edges: m.edges })
