 ...
 * ran `sweep brightdata.com` should not have to learn the file naming. Takes
 * the candidate file list rather than reading `runs/` itself — pulled out of
 * the CLI body (which used to run this matching inline at import time,
 * unreachable by a test process) the same way `recallForAnchor` was pulled
 * out of `recall.ts`: a pure function plus an `invokedDirectly` guard around
 * the argv/console body. `resolve`'s own history is two real bugs (the
 * newest-run sort used to open every match twice for nothing, per
...
/** Counts each string, most-frequent first. Shared by `--entities`, `by
 *  relation` and `by kind`. */
export function tally(xs: readonly string[]): [string, number][] {
...
`)
  ...
  // empty one, which returns `[]` and falls through to `resolve`'s "no run
  // matching" message, exactly as intended); this used to surface as a bare
  ...
  // `resolve` matches on substrings and gates on nothing but `.json`, so it
  // can hand back a file that is not a map at all — an investigator run from
  // demo-investigate.ts is {nodes, edges}, and `pnpm read demo` now reaches
  // one. Without this the first use of `r.entities` was a bare TypeError;
