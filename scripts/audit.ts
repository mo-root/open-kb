...
/** Sniff one file into rows: top-level entities (sweep, swarm) or the kernel
 *  wrapper's `result` — the same two shapes diff-runs reads. */
function readEntities(path: string): AuditEntityRow[] {
  const json = JSON.parse(readFileSync(path, "utf8")) as {
    entities?: AuditEntityRow[]
    result?: { entities?: AuditEntityRow[] }
  }
