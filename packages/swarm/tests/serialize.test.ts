...
const search: SearchPort = {
  async search(queries) {
    return [...new Set(queries)].map((query) => ({ query, hits: HITS, ok: true, usd: 0.001, ms: 1 }))
  },
}
  ...
  it("a run ending lead-finished-after-refusal serializes the exchange completely", async () => {
    const lead = new MockLanguageModelV4({
      doGenerate: async ({ prompt }) => {
        ...
        // The refusal's price, paid: one page, on the turn the refusal bought.
        if (turn === 3)
          return reply(
            call("g1", "fetch", { urls: ["https://rival.com"], mode: "direct", why: "open the host the seed only listed" }),
            false,
          )
    ...
    // the refused conclusion — and the four fields that say what the refusal
    // asked for, what it got, WHICH work that was, and which rule let the
    // finish through. A reader can hold the lead to them: one page was bought
    // on the turn the refusal bought, and `stood: "work"` is the run saying
    // that is why it ended.
    expect(report.scorecard.gate.refusals).toBe(1)
    expect(report.scorecard.gate.workAnswered).toBe(1)
    expect(report.scorecard.gate.stood).toBe("work")
    // Both halves of the receipt carry the facts of their own event. The page
    // half's number is the EXTRACTED text — what the model read, the string
    // the digest is taken over — not the raw bytes and not the slice returned
    // to the tool. Without it `page:<url>` said only that something readable
    // came back, which is equally true of a market's pricing table and of a
    // 210-character stub one character over core's THIN_TEXT floor: the exact
    // residue class the receipt exists to expose.
    expect(report.scorecard.gate.answeredBy).toEqual(["page:https://rival.com(266c)"])
    // ZERO, beside one landing: the run's only mission is the SEED, which the
    // harness wrote, so the lead owed against nothing and the page it bought
    // is the whole of what answered.
    expect(report.scorecard.gate.workAtRefusal).toBe(0)
    expect(run.landings).toHaveLength(1)
    expect(run.landings[0]!.mission.dedupeKey).toBe("orient:anchor.com")
