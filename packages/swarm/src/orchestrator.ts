...
export async function runSwarm(opts: SwarmOptions): Promise<SwarmRun> {
  ...
  const hooks: AgentHooks = {
    ...
    onTool: (i) => {
      ...
      emit({
        ...
        ms: i.ms,
        ok: i.ok,
        // Carried whether or not `ok` is false: a call that landed nine claims
        // and refused one still refused one, and the span log is the only copy
        // of that fact once the run ends.
  ...
  /**
   * THE LEAD'S WORK LOG — the whole of what the finish gate charges a refusal
   * against, and the receipt it records for what one bought. Append-only, in
   * real time, from exactly two places: a page the lead's own fetch tool
   * gained (below) and a mission THE LEAD COMMISSIONED coming home (in
   * `launch`'s settle). Nothing else may push here — the slice
   * `answeredBy` takes is only exact while every entry lands at the moment its
   * event does.
   *
   * Both halves are narrow on purpose, and the narrowing is the fix:
   *
   *   - Not every fetch. `onPageGained` fires only where the sniffer said
   *     `found` at page tier AND the run held neither that ADDRESS nor that
   *     READING. A dead host resolves `{httpStatus: 0, usd: 0}` at $0.00; an
   *     alias or a redirect onto a page already read is not a page; a second
   *     read of an address already visited is not a page however its bytes
   *     move; and a soft 404 echoing the path it was handed is one reading
   *     however many paths it is handed.
   *   - Not every landing. `DEFAULT_FAMILY_FLOOR = 4` is ON by default, the
   *     seed dig is the harness's, and a sweep handoff mints its own board —
   *     so a run-wide landing count let a lead that never spawned, fetched or
   *     searched read `stood: "work"` off missions the HARNESS wrote.
   *     `control.commissionedBy` is the discriminator, written where each
   *     mission is pushed — and NEITHER free verb of review rewrites it: a
   *     re-rank re-stamps only an unreviewed row, and a kill hands the key
   *     back to the board without handing back the name it was pushed under
   *     (tools-control.ts `reviewTool`, `RunControl.commissionerBeforeKill`).
   *
   * Each entry is also the RECEIPT for what a refusal bought, so it carries
   * the facts of its own event: `mission:<key>(<status>,+<nodes>)` for a
   * landing, since the charge deliberately does not look at either, and
   * `page:<url>(<chars>c)` for a page, where chars is the extracted text the
   * model actually read — the charge does not look at THAT either, and a
   * receipt that cannot tell a pricing table from a 210-character stub over
   * the THIN_TEXT floor is not auditable.
   */
  const leadWork: string[] = []
  ...
  const lead = runLead({
    ...
    onPageGained: (url, chars) => leadWork.push(`page:${url}(${chars}c)`),
  })
  ...
  const seed = seedMission(opts.domain)
  {
    ...
    for (const tier of ladder.slice(ladder.indexOf(seed.tier))) {
      ...
      // The one commission written before any model runs. It pushes straight
      // to the board rather than through spawnTool, so it records its own
      // commissioner here — and it is NOT the lead's: the finish gate must not
      // let a lead pay for a question it never asked.
      control.commissionedBy.set(seed.dedupeKey, "seed")
  ...
  const launch = (mission: BoardRow, claimId: string) => {
    ...
    const p: Promise<Wake> = runInvestigator(mission, deps)
      ...
      .then(async (digest) => {
        ...
        // And the gate's half of that record: only if the LEAD commissioned
        // it. A missing entry reads as "not the lead" — the direction that
        // cannot invent work the lead did not ask for.
        //
        // The entry carries the digest's OWN facts, because the charge cannot.
        // "A landing counts whatever its status" is the right rule — a market
        // that will not answer must not trap a lead, and status-gating the
        // charge hands the outcome to a race — but under a bare
        // `mission:<key>` receipt a lane that crashed on its first model call
        // (`{status: "crashed", added: {nodes: 0}, spentUsd: 0}`) read exactly
        // like one that came home with forty nodes. Same charge, different
        // receipt: `mission:rivals-eu(crashed,+0)` beside
        // `mission:rivals-eu(done,+7)` lets a reader see which it was.
        if (control.commissionedBy.get(mission.dedupeKey) === "lead") {
          leadWork.push(`mission:${mission.dedupeKey}(${digest.status},+${digest.added.nodes})`)
        }
        // Added PLUS merged, and the two halves of one fraction are the reason.
        // `pageTierNodes` is read off the map's writer stamps and counts every
        // live node this writer touched — merges included. Feeding the ledger
        // `added` alone put the numerator and the denominator on different
        // definitions, so the web overview could render "3 of 2 on page
        // evidence" — the `{f.pageTierNodes} of {f.nodesAdded} on page
        // evidence` span in packages/web/components/kb/KbOverview.tsx — the
        // moment a mission confirmed more page-tier hosts than it discovered.
        // The yield curve is unaffected: the `yieldHistory` expression in
        // `scorecardInput()` reads `digest.added.nodes` directly, never
        // through here.
        families.landed(mission.dedupeKey, digest.added.nodes + digest.merged.nodes)
        say(
          `lane frees: ${mission.lens} (${mission.dedupeKey}) — ${digest.status}, ` +
            `+${digest.added.nodes} nodes` +
            (digest.merged.nodes > 0 ? ` (${digest.merged.nodes} it confirmed)` : "") +
            `, $${actualUsd.toFixed(3)}`,
        )
  ...
  const scorecardInput = (): ScorecardInput => {
    ...
    return {
      ...
      wallMs,
      // The curve the lead reads exhaustion off. Same expression as before the
      // fix, a different number inside it: `added.nodes` is now the mission's
      // OWN new nodes, so a pane falling means the market is running out and
      // not, as it did, that the lanes are. Confirmations stay out of the
      // curve on purpose — the scorecard's question is new-node yield, and the
      // digest reports `merged` beside it for the lead to read.
  ...
  const openFamilyFloor = () => {
    ...
    const out = spawnTool(
      // A commissioner the gate's comment used to deny existed, and the one
      // that is ON BY DEFAULT. Its landings are the floor's, not the lead's,
      // however ordinary they look everywhere else downstream.
      { board, ledger, control, commissioner: "family-floor", onFamilyEvent: (e) => families.apply(e) },
  ...
  const openSweepSeeds = () => {
    ...
    const out = spawnTool(
      // Another: a board minted from a prior run file. The lead can review,
      // re-rank or kill any of it, and a handoff mission that lands is the
      // handoff's work whatever the lead did to the row first — a re-rank
      // moves a number, and a kill followed by a re-spawn under the same key
      // moves nothing at all (tools-control.ts `reviewTool`).
      { board, ledger, control, commissioner: "sweep", onFamilyEvent: (e) => families.apply(e) },
  ...
  for (;;) {
    ...
    if (winner.type === "mission") {
      ...
      // The lead's own copy, and the sentence the yield judgement rests on:
      // the mission's OWN writes, with confirmations named as confirmations.
      // "+0 nodes" and "+0 nodes, 3 already on the map when it started" are
      // different findings about the market and must not print the same.
      const line =
        `${winner.key} landed: ${d.status}, +${d.added.nodes} nodes` +
        (d.merged.nodes > 0 ? `, ${d.merged.nodes} it confirmed (already on the map when it started)` : "") +
