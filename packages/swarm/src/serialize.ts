    ...
    report: {
      ...
      scorecard: {
        ...
        gate: {
          ...
          // What the refusals asked for and what they were answered with. A
          // non-zero `refusals` beside `workAnswered: 0` is the run saying the
          // lead was asked for a page or a landing and produced neither.
          workAtRefusal: gate.workAtRefusal,
          workAnswered: gate.workAnswered,
          // And WHICH work, so `stood: "work"` is auditable rather than
          // merely asserted. Both halves carry the facts of their own event:
          // `page:<url>(<chars>c)` for a page the lead gained, chars being the
          // extracted text it read, and `mission:<dedupeKey>(<status>,+<nodes>)`
          // for a mission it commissioned coming home. A reader who thinks one
          // unrelated blog post is a poor answer to the objections can now see
          // that it was one unrelated blog post, and how much of one.
          answeredBy: gate.answeredBy,
          // And which rule let the accepted finish through, so `refusals: 0`
          // on a run with standing objections reads as the hatch it was
          // (`disarmed`, `turn-cap`, `budget-floor`) rather than as a pass.
          // Null when no finish was ever accepted.
          stood: gate.stood,
        },
      ...
      missions: run.landings.map((l) => ({
        ...
        // Both halves of the mission's own yield, and the reason a stored run
        // says which era it came from: an `added: {nodes, edges}` row is a
        // pre-fix global delta (runs/swarm-*-2026080[56]*.json), an
        // `added: {nodes}` beside `merged: {nodes}` is the mission's own.
        added: l.digest.added,
        merged: l.digest.merged,
        usd: l.actualUsd,
        atSec: l.atSec,
      })),
