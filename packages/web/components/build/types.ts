...
export interface PlanView {
  ...
  estimatedUsd: number;
  /**
   * The whole run's query ceiling, when one was set. Absent on a terminal run,
   * which has no clock and no ceiling.
   *
   * NOT `requested`, which is what a caller typed. This is what the DEPLOYMENT
   * can finish inside its function limit, and it is the number that explains a
   * narrow map: the same engine on the same anchor writes hundreds of queries
   * from a terminal. A reader who is not told this reads a short map as a weak
   * engine, which is the wrong lesson and the one this field exists to prevent.
   */
  ceiling?: number;
  /** The clock the ceiling was derived from, in seconds — the host's function
   *  limit less the margin the run reserves to record its own ending. */
  clockSeconds?: number;
}
...
export function readPlanned(v: unknown): PlanView | null {
  ...
  return {
    ...
    // Both null on an uncapped run, and `numOrUndef` turns that into absent —
    // which is what the card wants: no ceiling, no sentence about one.
    ceiling: numOrUndef(o.ceiling),
    clockSeconds: numOrUndef(o.clockSeconds),
  };
}
