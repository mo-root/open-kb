...
export function PlanCard({ plan }: { plan: PlanView }) {
  ...
  return (
    <div className="rounded-lg border border-sky-500/30 bg-sky-500/[0.04] p-5">
      ...
      {/* WHY THIS MAP IS THIS SIZE, said before the reader can wonder.
          The same engine on the same anchor writes hundreds of queries from a
          terminal, where nothing is timing it. Here it is sized to what this
          deployment's function limit can finish, and a reader who is not told
          that reads a narrow map as a weak engine. `ceiling` is absent on an
          uncapped run, so this renders only where it is true. */}
      {plan.ceiling !== undefined && (
        <p className="mt-3 text-[12px] leading-snug text-slate-400">
          ...
          — {plan.ceiling} question{plan.ceiling === 1 ? "" : "s"}
          {plan.clockSeconds !== undefined && <> is what fits in {plan.clockSeconds}s</>}, opening hand and
          every widening round together. Rank is the long pole: one page fetch and one model call per host
          found, so a question is really a host budget. Run it from a terminal and nothing stops it at{" "}
          {plan.ceiling}.
        </p>
      )}
