...
import { gateAnswer, gateExchange, nOfM, share, usdOfUsd } from "@/lib/scorecard-view";
...
function RunTelemetry({
   ...
   *  as "68% true". */
  grounding?: number;
  /** Both present only on a run a host was timing. See the sentence below the
   *  numbers for what they are for. */
  queryCeiling?: number;
  clockSeconds?: number;
  /** Hosts found and never judged, because the clock ran out mid-rank. */
  unjudged?: number;
}) {
  const items = [
  ...
  return (
    <Card className="reveal reveal-2">
      ...
      {/* THE SIZE OF THIS MAP IS A DECISION, and the reader is told whose.
          A run started from the browser has to finish inside the host's
          function limit, so it is given a query ceiling worked out from that
          limit; the same engine on the same anchor, run from a terminal with no
          clock, buys hundreds of queries and returns thousands of entities.
          Without this line the two look like the same tool having a bad day.
          Renders only where a ceiling actually existed — a terminal run has
          none, and says nothing rather than claiming it was unlimited. */}
      {queryCeiling !== undefined && (
        <p className="mt-4 border-t border-slate-800/80 pt-3 text-[12px] leading-snug text-slate-400">
          <span className="font-mono text-[10px] uppercase tracking-wider text-amber-300/90">
            sized to the host
          </span>{" "}
          — this run was allowed {queryCeiling} question{queryCeiling === 1 ? "" : "s"}
          {clockSeconds !== undefined && <>, the most that finishes inside its {clockSeconds}s clock</>}. Each
          one drags roughly a dozen hosts into the phase that fetches and judges every one of them, which is
          where a run&rsquo;s time actually goes.
          {unjudged !== undefined && unjudged > 0 && (
            <>
              {" "}
              {unjudged} host{unjudged === 1 ? "" : "s"} the sweep found {unjudged === 1 ? "was" : "were"}{" "}
              left unjudged when the clock ran out — the run shipped what it had rather than losing all of
              it.
            </>
          )}{" "}
          The same map from a terminal has no ceiling at all.
        </p>
      )}
    </Card>
  );
...
function CoveragePanel({ scorecard }: { scorecard: KbScorecard }) {
  ...
  // What the refusals cost and what ended the exchange. Two runs can print the
  // same objection sentences and mean opposite things — one lead bought a page,
  // the other restated — and a gate that declined to refuse at all is not the
  // same event as one that had nothing to say.
  const answer = gateAnswer(scorecard.gate);
      ...
      <div className="mt-3 border-t border-slate-800 pt-3">
        {exchange.spoke ? (
          ...
          <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-3">
            ...
            {answer && (
              <p className="mt-2 border-t border-amber-500/20 pt-2 font-mono text-[10px] text-amber-400/90">
                {answer}
              </p>
            )}
            {refused?.summary && (
          ...
          <p className="font-mono text-[10px] text-slate-500">
            the finish gate never refused{answer ? ` — ${answer}` : ""}.
          </p>
        )}
      </div>
    </Card>
  ...
  return (
    <div className="space-y-4" aria-label="knowledge base intelligence dashboard">
      ...
      <RunTelemetry
        ...
        queryCeiling={manifestNum(manifest, "queryCeiling")}
        clockSeconds={manifestNum(manifest, "clockSeconds")}
        unjudged={manifestNum(manifest, "unjudged")}
      />
