...
/**
 * `findKb` and `graphOf` are each pinned on their own (`kb-lookup.test.ts`,
 * `kb-from-run.test.ts`); this is the wiring between them that neither
 * covers — see the comment on `kb-lookup.test.ts` naming this route as one
 * of the four whose own test never exercised a missing id.
 */
...
function req(id: string): [Request, { params: Promise<{ id: string }> }] {
  return [new Request(`https://kb.test/api/kb/${id}/graph`), { params: Promise.resolve({ id }) }]
}
...
beforeAll(async () => {
  dir = await mkdtemp(path.join(tmpdir(), "openkb-kb-graph-route-"))
...
describe("GET /api/kb/[id]/graph", () => {
  ...
  it("answers with graphOf's node/edge shape, not the raw run", async () => {
    const res = await GET(...req(UUID))
    expect(res.status).toBe(200)
    const body = (await res.json()) as { nodes: unknown[]; edges: unknown[] }
    expect(Array.isArray(body.nodes)).toBe(true)
    expect(Array.isArray(body.edges)).toBe(true)
    // The anchor is always on the graph — graphOf's own guarantee, pinned in
    // kb-from-run.test.ts's zero-entity case; this only checks the route
    // reaches that function rather than, say, echoing the run.
