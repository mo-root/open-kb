...
export async function investigate(opts: InvestigateOptions): Promise<InvestigateResult> {
  ...
  const onStepFinish = (step: { usage?: { inputTokens?: number; outputTokens?: number } }) => {
    ...
    const usd = rate ? (inTok / 1e6) * rate.inUsdPerM + (outTok / 1e6) * rate.outUsdPerM : 0
    ctx.spans.emit({
      runId: ctx.runId,
      agentId: ctx.agentId,
  ...
  const result = await agent.generate({
    onStepFinish,
    // The context's signal reaches the tools' fetches; it has to reach the turns
    // as well, or a cancelled investigation stops buying pages and goes on
    // buying tokens.
