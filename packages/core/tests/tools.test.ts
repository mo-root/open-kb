...
describe("fetch tool", () => {
  ...
  it("hands the context's signal to the port on every fetch", async () => {
    // The port delegates its whole bound to the caller, so a caller that drops
    // the signal leaves a paid fetch answering to nothing: cancelling the run
    // stops the next thing and lets the one in flight keep spending. Invisible
    // in the response — only visible in what was handed over.
    const ctl = new AbortController()
    const c = { ...ctx(), signal: ctl.signal }
    const t = makeTools(c)
    await t.fetch.execute!(
      { urls: ["https://rival.com", "https://stripe.com/radar"], mode: "direct", why: "read two pages" },
      {} as never,
    )
    expect(c.fetch.calls).toHaveLength(2)
    expect(c.fetch.calls.every((call) => call.signal === ctl.signal)).toBe(true)
  })

  it("works unchanged for a caller that has no signal to give", async () => {
    // A script or a test has nothing to cancel with. It passes nothing and
    // falls back to the port's own backstop; it must not have to invent one.
    const c = ctx()
    const t = makeTools(c)
    const out = await t.fetch.execute!({ urls: ["https://rival.com"], mode: "direct", why: "read a page" }, {} as never)
    expect(out.results[0]!.status).toBe("found")
    expect(c.fetch.calls[0]!.signal).toBeUndefined()
  })

  it("gives the model the refusal the provider stated, rather than the generic guess", async () => {
    // `hintFor`-style sentences have to describe every way a host can answer
    // with nothing; the header names the one that happened.
    const c = ctx()
    c.fetch = new FakeFetch({
    ...
    const t = makeTools(c)
    const out = await t.fetch.execute!({ urls: ["https://stripe.com/radar"], mode: "unlocked", why: "read it" }, {} as never)
    const r = out.results[0]!
    // The verdict is the sniffer's, unchanged. The hint carries BOTH: the
    // provider's sentence says which way the host answered with nothing, and
    // the generic sentence after it says what to do next. Substituting one for
    // the other hands the model a diagnosis and takes away the advice — the
    // first draft of this fix did exactly that, and pinned it here.
    // packages/swarm/src/tools-paid.ts draws the same line, and says why.
    expect(r.reason).toBe("empty-body")
    expect(r.hint).toContain("requested site was blocked (upstream 403)")
    expect(r.hint).toContain("try a different page")
