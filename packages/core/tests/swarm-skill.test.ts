...
describe("swarm skill", () => {
  ...
  it("teaches the finish exchange priced in work, the gate's tail quoted byte for byte", () => {
    ...
    // The lesson, in the file rather than only in the code: the old tail
    // offered "carry them into unresolved verbatim; your next finish stands"
    // and both live refusals were answered with exactly that and no paid work
    // (runs/swarm-brightdata-com-202608060833 turn 19 → turn 20, seq 371→374;
    // runs/swarm-resend-com-202608060732 turn 22 → turn 23, seq 227→229). The
    // skill must not teach the sentence the gate no longer accepts.
    expect(raw).not.toContain("restate finish — it stands")
    // And it must teach the price the gate actually charges. "Fetches and
    // landings" was the wrong phrase for it: a fetch CALL completes for $0.00
    // against an unreachable host, and a page the run already holds re-buys
    // for the same nothing, so what counts is what the run came away holding.
    expect(raw).toContain("counts what the run GAINED, not sentences")
    expect(raw).toContain("a page you already hold or a search gains nothing")
    // And the one honest answer the price refuses, said where the lead reads
    // rather than only where the code lives. `addressKey` drops the query
    // deliberately — keeping it would leave `?q=1`, `?q=2`, `?q=3` as an
    // unbounded supply of fresh addresses on any host at $0 a fetch — so a
    // site that distinguishes pages BY QUERY ALONE folds, and a lead that
    // answers a refusal with the second one has bought real bytes it will not
    // be paid for. The decision is pinned in swarm's own suite; this keeps the
    // warning in the file the lead actually gets.
    expect(raw).toContain("one path is one page whatever its query")
    // The gate grants the answering turn at once and never parks the lead on a
    // landing, so a lead answering with a mission has to wait for it itself.
    expect(raw).toContain("let it land, then finish")
  })
  ...
  it("gives the lead its band, its re-entry, its review seat, and its ending", () => {
    ...
    // Both free verbs of review, and what neither of them buys. The skill used
    // to teach kill as an unconditional verb with no price attached, and the
    // gate charges the lead for landings it commissioned: re-ranking a harness
    // row does not commission it, and neither does killing one and spawning it
    // again under the same key (swarm's tools-control holds both rules).
    expect(raw).toContain("neither makes a harness row yours")
    // The band sentence has to be true of the DEFAULT path: the family floor
    // pushes p70/68/66/64/62 and a sweep handoff p61-70, so "everything above
    // priority 60 was ranked by you" was false the moment the floor shipped on
    // by default.
    // Kept wrap-insensitive on purpose: each pinned phrase sits inside one
    // line today, and a re-wrap that split one would fail this test for a
    // reason that has nothing to do with the lesson.
    expect(raw).not.toContain("Everything above priority 60 was")
    expect(raw).toContain("The 61–100 band is yours to rank")
    expect(raw).toContain("seeded by the harness")
  })
