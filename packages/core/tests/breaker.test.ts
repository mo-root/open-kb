...
// tool emits (empty-body, soft-404, thin-render — core/src/sniff.ts's own
// reason union, and tools-paid.ts's STRIKES set). The table never imports
// them: reasons are opaque text it repeats back, so the vocabulary can grow
...
describe("BreakerTable", () => {
  ...
  it("one strike is not enough", () => {
    const t = new BreakerTable()
    t.strike("stripe.com", "unlock", "empty-body")
  ...
  it("two strikes open the pair, and the sentence names host, mode, and reason", () => {
    ...
    expect(r.because).toBe("stripe.com refuses the unlock tier this run: two empty-body responses")
  })

  it("mixed reasons still name the last one", () => {
    const t = new BreakerTable()
    t.strike("adyen.com", "direct", "soft-404")
  ...
  it("strike reports the running count and whether the pair just opened", () => {
    const t = new BreakerTable()
    expect(t.strike("stripe.com", "unlock", "empty-body")).toEqual({ strikes: 1, open: false })
    expect(t.strike("stripe.com", "unlock", "empty-body")).toEqual({ strikes: 2, open: true })
  })
  ...
  it("stays open past two strikes and keeps counting honestly", () => {
    ...
    const r = t.open("stripe.com", "unlock")
    expect(r.open).toBe(true)
    expect(r.because).toBe("stripe.com refuses the unlock tier this run: three empty-body responses")
  })
  ...
  it("falls back to a digit once the count word list runs out", () => {
    ...
    const t = new BreakerTable()
    for (let i = 0; i < 7; i++) t.strike("stripe.com", "unlock", "empty-body")
    const r = t.open("stripe.com", "unlock")
    expect(r.because).toBe("stripe.com refuses the unlock tier this run: 7 empty-body responses")
  })
})
